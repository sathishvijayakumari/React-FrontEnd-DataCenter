...
export default class RealTimeTracking1 extends Component {
    ...
    animation = (rackId, status) => {
        this.setState({ rackID: rackId, assetID: "" });
        this.tableSlideAnimate("hide");
        $("#thermal").attr("checked", false);  
        $("#energy").attr("checked", false);  
        ...
        if (status === "show") {
            ...
            this.radioBtnChange("asset",rackId);
        } else if (status === "hide") {
            // $('input:radio')
            //     .removeAttr('checked')
            //     .removeAttr('selected')
        ...
        axios({ method: "GET", url: "/api/asset/racktracking?rackid=" + rackId })
            .then((response) => {
                if (response.status === 200 || response.status === 201) {
                    let data = response.data.asset;
                    if (data.length !== 0) {
                        console.log(data.length, "assetView =====>", response);
                        for (let i = 0; i < data.length; i++) {
    ...
    radioBtnChange = (val,rackid) => {
        ...
        if (val.length > 0) {
            ...
            checkVal = $("input[name='rack']:checked").val();
        ...
        if (checkVal === "asset") {
            ...
            $("#asset_details").css("display", "none");
    ...
    render() {
        const { error, message, series, rackID, assetID } = this.state;
        return (
            <div style={{
                ...
                <div id="screen" style={{
                    ...
                    <div style={{ margin: "5px 26px" }}>
                        ...
                        <div id="screen_content" style={{ marginTop: "30px" }}>
                            ...
                            <div id="asset_details" style={{ display: "flex" }}>
                                <p className='rack-asset-info' style={{ marginLeft: "30%" }}>
                                    <button id="4" className="fancy-button"
